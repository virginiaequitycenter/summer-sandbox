...
author: "Helena Lindsay, Michele Claibourn"
date: "11/29/2021"
output:
  html_document: 
    toc: TRUE
...
walk <- read_csv("../data/eastern_walk.csv")
walk <- walk %>% 
...
```{r, include = F}
eastern_maps <- left_join(eastshore_blkgps, walk, by = c('GEOID'= "FIPS_BLKGP"))
eastern_maps <- st_transform(eastern_maps, crs = 4326) # to WGS84, given error
```

### Walkability

```{r}
pal <- colorNumeric("plasma", reverse = TRUE, domain = eastern_maps$walkability_index)

leaflet(eastern_maps) %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(data = eastern_maps,
              ...
              popup = paste0("Tract Number: ", eastern_maps$GEOID, "<br>",
                             "Walkability Index: ", round(eastern_maps$walkability_index, 2))) %>%
  addLegend("bottomright", pal = pal, values = eastern_maps$walkability_index,
...
```{r}
pal <- colorNumeric("plasma", reverse = TRUE, domain = eastern_maps$intersection_density)
...
leaflet(eastern_maps) %>%
  ...
  addPolygons(data = eastern_maps,
              ...
              highlight = highlightOptions(weight = 2, fillOpacity = 0.8, bringToFront = T),
              popup = paste0("Tract Number: ", eastern_maps$GEOID, "<br>",
                             "Intersection Density: ", round(eastern_maps$intersection_density, 2))) %>%
  addLegend("bottomright", pal = pal, values = eastern_maps$intersection_density,
...
```{r}
pal <- colorNumeric("plasma", reverse = TRUE, domain = eastern_maps$employment_mix)
...
leaflet(eastern_maps) %>%
  ...
  addPolygons(data = eastern_maps,
              ...
              highlight = highlightOptions(weight = 2, fillOpacity = 0.8, bringToFront = T),
              popup = paste0("Tract Number: ", eastern_maps$GEOID, "<br>",
                             "Employment Mix: ", round(eastern_maps$employment_mix, 2))) %>%
  addLegend("bottomright", pal = pal, values = eastern_maps$employment_mix,
...
```{r}
pal <- colorNumeric("plasma", reverse = TRUE, domain = eastern_maps$emp_housing_mix)
...
leaflet(eastern_maps) %>%
  ...
  addPolygons(data = eastern_maps,
              ...
              highlight = highlightOptions(weight = 2, fillOpacity = 0.8, bringToFront = T),
              popup = paste0("Tract Number: ", eastern_maps$GEOID, "<br>",
                             "Employment-Housing Mix: ", round(eastern_maps$emp_housing_mix, 2))) %>%
  addLegend("bottomright", pal = pal, values = eastern_maps$emp_housing_mix,
