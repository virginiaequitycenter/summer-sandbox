---
title: "LEHD LODES"
subtitle: "Eastern Shore Region Block Data"
author: "Lee LeBoeuf"
date: "07/06/2021"
output: 
  html_document:
...
leaflet(east_lodes) %>% 
  ...
  addPolygons(data = east_lodes,
              ...
              popup = paste0("Block Number: ", east_lodes$BLOCKCE10, "<br>",
...
### Wages
```{r}
pal <- colorNumeric("plasma", reverse = TRUE, domain = east_lodes$lowwage_p)
leaflet(east_lodes) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = east_lodes,
...
```{r}
# High wage jobs
pal <- colorNumeric("plasma", reverse = TRUE, domain = east_lodes$higwage_p)
leaflet(east_lodes) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = east_lodes,
