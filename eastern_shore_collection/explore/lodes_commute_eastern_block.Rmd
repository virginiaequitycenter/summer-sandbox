---
title: "LEHD LODES"
subtitle: "Eastern Shore Region 2018 Commuting Data"
author: "Lee LeBoeuf"
date: "07/29/2021"
output: 
  html_document:
...
lodes <- read.csv("lodes_commute_eastern_block.csv")
...
east_lodes <- merge(shape, lodes, by = 'h_geocode', all.x = T)
```
...
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = east_lodes$avgc_within40)
leaflet(east_lodes) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = east_lodes,
              ...
              popup = paste0("GEOID: ", east_lodes$h_geocode, "<br>",
                             "Average commute (mi): ", round(east_lodes$avgc_within40, 2))) %>% 
  addLegend("bottomright", pal = pal, values = east_lodes$avgc_within40, 
            title = "Average commute (mi)", opacity = 0.7)
```
...
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = east_lodes$avgc_25_employees)
leaflet(east_lodes) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = east_lodes,
              ...
              popup = paste0("GEOID: ", east_lodes$h_geocode, "<br>",
                             "Average commute (mi): ", round(east_lodes$avgc_25_employees, 2))) %>% 
  addLegend("bottomright", pal = pal, values = east_lodes$avgc_25_employees, 
            title = "Average commute (mi)", opacity = 0.7)
```
...
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = east_lodes$avgc_workinRegion)
leaflet(east_lodes) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = east_lodes,
              ...
              popup = paste0("GEOID: ", east_lodes$h_geocode, "<br>",
                             "Average commute (mi): ", round(east_lodes$avgc_workinRegion, 2))) %>% 
  addLegend("bottomright", pal = pal, values = east_lodes$avgc_workinRegion, 
            title = "Average commute (mi)", opacity = 0.7)
```
