---
title: "LEHD LODES"
subtitle: "Eastern Shore Region Block Group Data"
author: "Lee LeBoeuf"
date: "07/06/2021"
output: 
  html_document:
...
lodes <- read.csv("lodes_employment_eastern_blkgr.csv")
...
## Mapping the data 
```{r, echo = FALSE}
shape <- readRDS('eastshore_blkgps.RDS')
shape <- st_transform(shape, crs = 4326) # to WGS84, given error
shape <- shape %>% dplyr::rename(w_blkgr = GEOID)
is.element(lodes$w_blkgr, shape$w_blkgr) %>%
  all()
eastern_lodes <- merge(shape, lodes, by = 'w_blkgr', all.x = T)
```
...
leaflet(eastern_lodes) %>% 
  ...
  addPolygons(data = eastern_lodes,
              ...
              popup = paste0("Block Group: ", eastern_lodes$BLKGRPCE, "<br>",
