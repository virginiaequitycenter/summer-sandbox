...
# Michele Claibourn, updated by Lee LeBoeuf
# Created: 2023-07-28

library(tidyverse)
library(readxl)

region <- c("51001", "51131")
region3 <- c("001", "131")
...
# destfile <- "county_health_rankings.xlsx"
# download.file(url, destfile)

county_life <- read_excel("county_health_rankings.xlsx", sheet = 5, skip = 1) 
...
# Save
write.csv(region_life, "lifeexp_eastern_county.csv", row.names = F) 
...
# destfile <- "tract_life_expectancy.xlsx"
# download.file(url, destfile)

tract_expectancy <- read_excel("tract_life_expectancy.xlsx")
...
# Save
write.csv(region_tract_life, "lifeexp_eastern_tract.csv", row.names = F) 
