...
crs(eastern_tracts) # NAD83

# align crs in tract polygons and nlcd raster
eastern_tracts2 <- st_transform(eastern_tracts, proj4string(nlcd_img))

# a. Crop Land Cover
nlcd_img_est <- crop(nlcd_img, eastern_tracts2)
plot(nlcd_img_est)

# Check: plot raster with eastern shore boundaries
nlcd_df <- as.data.frame(nlcd_img_est, xy = TRUE)
...
ggplot() +
  ...
  geom_sf(data = eastern_tracts2, color = "red", fill = NA)

# b. Crop Impervious Surfaces
# imperv_img_cvl2 <- mask(imperv_img_cvl, cville_tracts2)
imperv_img_est <- crop(imperv_img, extent(eastern_tracts2))
plot(imperv_img_est)

# Check: plot raster with cvl boundaries
imp_df <- as.data.frame(imperv_img_est, xy = TRUE)
...
tree_img_est <- crop(tree_img, extent(eastern_tracts2))
plot(tree_img_est)

# Check: plot raster with cvl boundaries
tree_df <- as.data.frame(tree_img_est, xy = TRUE)
...
# align crs in tract polygons and nlcd raster
eastern_tracts2 <- st_transform(eastern_tracts, proj4string(nlcd_img_est))
eastern_blkgps2 <- st_transform(eastern_blkgps, proj4string(nlcd_img_est))
eastern_blocks2 <- st_transform(eastern_blocks, proj4string(nlcd_img_est))

# Extract Land Cover ----
land_extract_tracts <- raster::extract(nlcd_img_est, eastern_tracts2, df = TRUE)
...
# block groups
land_extract_blkgps <- raster::extract(nlcd_img_est, eastern_blkgps2, df = TRUE)
...
# blocks
land_extract_blocks <- raster::extract(nlcd_img_est, eastern_blocks2, df = TRUE)
...
# imperv_extract_tracts <- raster::extract(imperv_img_est, eastern_tracts2, df = TRUE)
...
imperv_summary_tracts <- raster::extract(imperv_img_est, eastern_tracts2, df = TRUE, 
                                         fun = mean, na.rm = TRUE,
                                         sp = TRUE)

imperv_summary_blkgps <- raster::extract(imperv_img_est, eastern_blkgps2, df = TRUE, 
                                         fun = mean, na.rm = TRUE,
                                         sp = TRUE)

imperv_summary_blocks <- raster::extract(imperv_img_est, eastern_blocks2, df = TRUE, 
                                         fun = mean, na.rm = TRUE,
                                         sp = TRUE)
...
# tree_extract_tracts <- raster::extract(tree_img_est, eastern_tracts2, df = TRUE)
...
# use the mean as the summary function for the moment
tree_summary_tracts <- raster::extract(tree_img_est, eastern_tracts2, df = TRUE, 
                                       fun = mean, na.rm = TRUE,
                                       sp = TRUE)

tree_summary_blkgps <- raster::extract(tree_img_est, eastern_blkgps2, df = TRUE,
                                       fun = mean, na.rm = TRUE,
                                       sp = TRUE)

tree_summary_blocks <- raster::extract(tree_img_est, eastern_blocks2, df = TRUE,
                                       fun = mean, na.rm = TRUE,
                                       sp = TRUE)
...
# Write csv
write_csv(tracts, "data/nlcd_eastern_tracts.csv")
write_csv(blkgps, "data/nlcd_eastern_blkgps.csv")
write_csv(blocks, "data/nlcd_eastern_blocks.csv")
