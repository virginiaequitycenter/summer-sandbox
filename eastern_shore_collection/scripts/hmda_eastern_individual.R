...
# This script keeps data at the individual mortgage level--use the "hmda_eastern_tract.R" script
...
# the eastpost2017 dataframe below with whatever the new object is called. 

# Creating necessary filtering strings
eastcounties <- c("51001", "51131") # Data post-2017 needs
...
# Binding all the data together! (The eastpre2018 data frame was created below, but should never need to be created again)
# In the future, run the above code and simply r bind the new data frame to the product below (eastall)
eastall <- bind_rows(eastpost2017, eastpre2018)

# Writing out the data 
write.csv(eastall, "hmda_eastern_individual.csv", row.names = F)
...
eastpre2018$county_code <- paste0(eastpre2018$state_code, eastpre2018$county_code)
...
eastpre2018$census_tract <- ifelse(!is.na(eastpre2018$county_code) & !is.na(eastpre2018$census_tract_number), 
                                   paste0(eastpre2018$county_code, eastpre2018$census_tract_number), NA)
...
eastpre2018$county_code <- as.numeric(eastpre2018$county_code)
eastpre2018$census_tract <- as.numeric(eastpre2018$census_tract)
