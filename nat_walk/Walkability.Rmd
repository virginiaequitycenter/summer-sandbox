...
## D3B{.tabset}
Street intersection density (pedestrian-oriented intersections). Higher intersection density is correlated with more walk trips.
...
## D2B_E8MIXA {.tabset}
The mix of employment types in a block group (such as retail, office, or industrial). Higher values correlate with more walk trips.

### County
```{r, message=F, warning=F}

cville_walk%>%
  ggplot(aes(x = D2B_E8MIXA)) + 
    geom_histogram() + 
    facet_wrap(~COUNTYFP)+
    theme(strip.background = element_blank())
```
