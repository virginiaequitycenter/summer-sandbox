...
# This script reads in already cleaned individual-level data for mortages and mortgage applications in the Charlottesville region, 
# and uses it to create tract-level summaries of only home purchase mortgages in the area. 
# All mortgages are filtered to only include home purchase mortgages because those were of primary
# interest, but other mortgages can be included by skipping to the section of code below that begins with, "ALL MORTGAGE APPS"
# Tract level summaries include variables like the raw counts of different loan types (conventional, FHA insured, etc.), 
# the counts of loan applications from different racial groups, the median loan amounts and applicant income. 
# Note: income and loan amount variables seem highly unreliable and prone to entry errors--many of the values seem impossible. 
...
# Therefore, I only include the median values and not the averages in the data. 
...
# summer on a year and a half delay), use the "hmda_cville_individual.R" script to clean the new data and bind it with the old 
...
cvldat$multiracial <- ifelse(!is.na(cvldat$applicant_race_1) & !is.na(cvldat$applicant_race_2) | !is.na(cvldat$applicant_race_3) 
                                                               | !is.na(cvldat$applicant_race_4) | !is.na(cvldat$applicant_race_5), 1, 0)
...
cvltractf <- left_join(cvldattract, popdat, by = c("census_tract", "year"), keep = FALSE)
...
# This line creates a variable to code applicants as multiracial by counting the number of 
# applicants who had selected multiple racial identities (i.e., not just White, Black, etc.)
cvldatall$multiracial <- ifelse(!is.na(cvldatall$applicant_race_1) & !is.na(cvldatall$applicant_race_2) | !is.na(cvldatall$applicant_race_3) 
                             | !is.na(cvldatall$applicant_race_4) | !is.na(cvldatall$applicant_race_5), 1, 0)
# Creating tract level summaries
cvldatalltract <- cvldatall %>%
  group_by(census_tract, year) %>%
  summarise(conventional = sum(loan_type == "Conventional"),
            fha_insured = sum(loan_type == "FHA insured"),
            va_guaranteed = sum(loan_type == "VA guranteed"),
...
cvltractallf <- left_join(cvldatalltract, popdat, by = c("census_tract", "year"), keep = FALSE)
cvltractallf <- cvltractf[-which(is.na(cvltractallf$census_tract)),]

# Writing out tract-level data for 2007-2020
write.csv(cvltractallf, "hmda_cville_tract.csv", row.names = F)
