...
invisible(lapply(list('tidyverse','naniar', 'sf','stringr', 'leaflet'),
...
# November 22, 2021 url: https://www.ncei.noaa.gov/pub/data/cirs/climdiv/climdiv-tmincy-v1.0.0-20211104

# Import data from source ----
# if (!dir.exists("dataraw")) {dir.create("dataraw")}
# url <- "https://www.ncei.noaa.gov/pub/data/cirs/climdiv/climdiv-tmincy-v1.0.0-20211104"
# download.file(url = url,
#               destfile = paste(getwd(), "/dataraw/", "climdiv-tmincy.txt", sep = ""),
#               mode = "wb")

va_noaa <- read.table(file = "dataraw/climdiv-tmincy.txt", header = FALSE)
head(va_noaa)

cville_noaa <- rename(va_noaa, fipsdivelemyear=V1, Janmin=V2, Febmin=V3, Marmin=V4, Aprmin=V5, Maymin=V6, Junmin=V7, 
...
# in July of 2021, the url ended with "0.0-20210707". When I downloaded data in October of 2021, I had to update the date to "0.0-20211006"
# To check what the right ending is, go to the link above and look at the file names. 
# November 22, 2021 url: climdiv-tmaxcy-v1.0.0-20211104
...
va_noaa2 <- read.table(file = "dataraw/climdiv-tmaxcy.txt", header = FALSE)
head(va_noaa)

# Rename variables of interest
cville_noaa2 <- rename(va_noaa2, fipsdivelemyear=V1, Janmax=V2, Febmax=V3, Marmax=V4, Aprmax=V5, Maymax=V6, Junmax=V7, 
...
# Note: The date at the end of the url below may need to be updated if data are downloaded at a later date. When Tolu initially downloaded data 
# in July of 2021, the url ended with "0.0-20210707". When I downloaded data in October of 2021, I had to update the date to "0.0-20211006"
# November 22, 2021 url: climdiv-pcpncy-v1.0.0-20211104
...
# Rename variables of interest
cville_noaa3 <- rename(va_noaa3, fipsdivelemyear=V1, Janpcp=V2, Febpcp=V3, Marpcp=V4, Aprpcp=V5, Maypcp=V6, Junpcp=V7, 
...
noaa_cville <- select(cville_noaa, -c(state, fipsdivelemyear, elem)) %>% 
  left_join(select(cville_noaa2, -c(state, fipsdivelemyear, elem)), by = c("county", "year")) %>% 
  left_join(select(cville_noaa3, -c(state, fipsdivelemyear, elem)), by = c("county", "year")) %>% 
  select(county, year, Avg_Tempmin, Avg_Tempmax, Avg_monthlypcp, Tot_yearlypcp,
         everything())

# Save to csv
write_csv(noaa_cville, file = "data/noaa_cville_county.csv")
