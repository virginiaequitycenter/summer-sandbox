...
popdat <- read.csv("../data/acs_tract_cville.csv")

shape <- readRDS('../data/cville_tracts.RDS')
shape$GEOID <- as.numeric(shape$GEOID)
shape <- st_transform(shape, crs = 6639)

healthdat <- read.csv("../data/cdcplaces_cville_tract.csv")
healthdat <- healthdat %>%
  rename(GEOID = locationname)
air <- read.csv("../data/airquality_cville_tract.csv")
air <- air %>%
  rename(GEOID = gid)


edat <- read.csv('../data/lodes_employment_cville_tracts.csv')
edat <- edat %>%
  rename(GEOID = w_tract)

lead <- read.csv("../data/leadexposure_cville_tract.csv")

surface <- read.csv("../data/landsat8_cville_tracts.csv")
...
What's the deal with the white spaces? ^[The white spaces in the reshaped cartogram are just a result of newly scaled census tracts not fitting together perfectly. They do not represent a specific tract/geographic area.]
