...
lodesresidents <- read.csv("lodes_commute_cville_block.csv")
lodesworkers <- read.csv("lodes_workerscommute_cville_block.csv")
...
### Charlottesville Area residents {.tabset}
```{r}
lodesresidents %>% select(avgc_all, avgc_within40, avgc_25_employees, avgc_workinRegion, medc_all, medc_within40, medc_25_employees, medc_workinRegion) %>% 
...
### Charlottesville Area workers
```{r}
lodesworkers %>% select(avgc_allworkers, avgc_livewithin40, avgc_25_res, medc_allworkers, medc_livewithin40, medc_25_res, perc_workers_liveoutsideCville) %>% 
  select(where(~is.numeric(.x))) %>% 
  as.data.frame() %>% 
  stargazer(., type = "text", title = "Summary Statistics", digits = 2,
            summary.stat = c("mean", "sd", "min", "median", "max"))
...
### Charlottesville Area residents {.tabset}
```{r}
longr <- lodesresidents %>% select(c(h_geocode, avgc_all, avgc_within40, avgc_25_employees, avgc_workinRegion, medc_all, medc_within40, medc_25_employees, medc_workinRegion)) %>% 
...
### Charlottesville Area workers
```{r}
longw <- lodesworkers %>% select(c(w_geocode, avgc_allworkers, avgc_livewithin40, avgc_25_res, medc_allworkers, medc_livewithin40, medc_25_res)) %>% 
...
shape <- shape %>% dplyr::rename(geocode = GEOID10)
lodesworkers <- lodesworkers %>% rename(geocode = w_geocode)
lodesresidents <- lodesresidents %>% rename(geocode = h_geocode)
cvl_lodes <- merge(shape, lodesresidents, by = 'geocode', all.x = T)
cvl_lodesfull <- merge(cvl_lodes, lodesworkers, by = 'geocode', all.x = T)
```
...
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = cvl_lodesfull$avgc_all)
leaflet(cvl_lodesfull) %>% 
  ...
  addPolygons(data = cvl_lodesfull,
              ...
              popup = paste0("GEOID: ", cvl_lodesfull$geocode, "<br>",
                             "Average commute (mi): ", round(cvl_lodesfull$avgc_all, 2))) %>% 
  addLegend("bottomright", pal = pal, values = cvl_lodesfull$avgc_all, 
            title = "Average commute (mi)", opacity = 0.7)
```

#### All Cville area workers {.tabset}
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = cvl_lodesfull$avgc_allworkers)
leaflet(cvl_lodesfull) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = cvl_lodesfull,
              fillColor = ~pal(avgc_allworkers),
              weight = 1,
              opacity = 1,
              color = "white", 
              fillOpacity = 0.6,
              ...
              popup = paste0("GEOID: ", cvl_lodesfull$geocode, "<br>",
                             "Average commute (mi): ", round(cvl_lodesfull$avgc_allworkers, 2))) %>% 
  addLegend("bottomright", pal = pal, values = cvl_lodesfull$avgc_allworkers, 
            title = "Average commute (mi)", opacity = 0.7)
```
...
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = cvl_lodesfull$avgc_within40)
leaflet(cvl_lodesfull) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = cvl_lodesfull,
              ...
              popup = paste0("GEOID: ", cvl_lodesfull$geocode, "<br>",
                             "Average commute (mi): ", round(cvl_lodesfull$avgc_within40, 2))) %>% 
  addLegend("bottomright", pal = pal, values = cvl_lodesfull$avgc_within40, 
            title = "Average commute (mi)", opacity = 0.7)
```

#### Cville area workers who live within 40 miles of work {.tabset}
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = cvl_lodesfull$avgc_livewithin40)
leaflet(cvl_lodesfull) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = cvl_lodesfull,
              fillColor = ~pal(avgc_livewithin40),
              weight = 1,
              opacity = 1,
              color = "white", 
              fillOpacity = 0.6,
              ...
              popup = paste0("GEOID: ", cvl_lodesfull$geocode, "<br>",
                             "Average commute (mi): ", round(cvl_lodesfull$avgc_livewithin40, 2))) %>% 
  addLegend("bottomright", pal = pal, values = cvl_lodesfull$avgc_livewithin40, 
            title = "Average commute (mi)", opacity = 0.7)
```
...
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = cvl_lodesfull$avgc_workinRegion)
leaflet(cvl_lodesfull) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = cvl_lodesfull,
              ...
              popup = paste0("GEOID: ", cvl_lodesfull$geocode, "<br>",
                             "Average commute (mi): ", round(cvl_lodesfull$avgc_workinRegion, 2))) %>% 
  addLegend("bottomright", pal = pal, values = cvl_lodesfull$avgc_workinRegion, 
            title = "Average commute (mi)", opacity = 0.7)
```

### Percent of workers who live outside the Charlottesville region
```{r}
pal <- colorNumeric("plasma", reverse = T, domain = cvl_lodesfull$perc_workers_liveoutsideCville)
leaflet(cvl_lodesfull) %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = cvl_lodesfull,
              fillColor = ~pal(perc_workers_liveoutsideCville),
              weight = 1,
              opacity = 1,
              color = "white", 
              fillOpacity = 0.6,
              ...
              popup = paste0("GEOID: ", cvl_lodesfull$geocode, "<br>",
                             "Percent of workers: ", round(cvl_lodesfull$perc_workers_liveoutsideCville, 2))) %>% 
  addLegend("bottomright", pal = pal, values = cvl_lodesfull$perc_workers_liveoutsideCville, 
