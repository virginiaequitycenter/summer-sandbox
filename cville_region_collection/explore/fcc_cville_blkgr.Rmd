...
cvldat <- read.csv("fcc_broadband_cville_blkgr.csv")
```
...
## Visual distribution 
```{r}
cvldat %>% select(c(Blkgr, consproviders, busproviders, avgMaxAdDown, avgMaxAdUp, p_under25.3mbps)) %>% 
...
shape <- shape %>% dplyr::rename(Blkgr = GEOID)
mapdat <- merge(shape, cvldat, by = 'Blkgr', all.x = T)
```
