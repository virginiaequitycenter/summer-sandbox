...
cvl_lodes_res <- read.csv("lodes_rescommutepatterns_cvlcounty.csv")
cvl_lodes_work <- read.csv("lodes_workerscommutepatterns_cvlcounty.csv")
```
...
```{r}
cvl_lodes_all2$work <- ifelse(cvl_lodes_all2$h_county %in% cvlfips, NA, cvl_lodes_all2$commuterstoRegion)
...
leaflet(cvl_lodes_all2) %>% 
  ...
  addPolygons(data = cvl_lodes_all2,
              ...
              popup = paste0("County: ", cvl_lodes_all2$NAME, "<br>",
                             "Number of commuters: ", cvl_lodes_all2$commuterstoRegion)) %>% 
