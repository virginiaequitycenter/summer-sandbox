...
# main data
cvldat <- read.csv("../data/fcc_broadband_cville_block.csv")
cvldat <- cvldat %>% 
  mutate(BlockCode = as.character(BlockCode))

#  spatial data
shape <- readRDS('../data/cville_blocks.RDS')
...
cvldat %>% select(-c(BlockCode, Blkgr, tract)) %>% 
...
```{r, echo = FALSE}
mapdat <- shape %>% 
  left_join(cvldat, by = c("GEOID10" = "BlockCode"))
```
