...
cvldat <- read.csv("fcc_broadband_cville_block.csv")
```
...
glimpse(cvldat)
cvldat %>% select(consproviders, busproviders, avgMaxAdDown, avgMaxAdUp, under25.3mbps) %>% 
...
## Visual distribution 
```{r}
cvldat %>% select(c(BlockCode, consproviders, busproviders, avgMaxAdDown, avgMaxAdUp)) %>% 
...
shape <- shape %>% dplyr::rename(BlockCode = GEOID10)
mapdat <- merge(shape, cvldat, by = 'BlockCode', all.x = T)
```
