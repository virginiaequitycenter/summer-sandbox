...
# invisible(lapply(list('rgdal', 'lodes', 'psych', 'reshape2', 'transformer', 'gifski', 'dygraphs', 'xtx')))
...
# main data
cvldat <- read.csv("../data/daymet_cville_county.csv")
cvldat <- cvldat %>% 
  mutate(GEOID = as.character(GEOID))
cvldatblkgr <- read.csv("../data/daymet_cville_blkgrps.csv")
cvldatblkgr <- cvldatblkgr %>% 
  mutate(GEOID = as.character(GEOID))
...
```{r, echo = FALSE}
mapdat <- left_join(shape, cvldatblkgr, by = 'GEOID')
```
...
cville_sf1x <- cbind(cville_sf1g, cville_sf1al, cville_sf1c, cville_sf1f,
                     cville_sf1l, cville_sf1n) %>%
