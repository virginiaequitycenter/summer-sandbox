...
nfhl_cville_tracts <- read_csv("../dataraw/nfhl_cville_tracts.csv")
nfhl_cville_blkgps <- read_csv("../dataraw/nfhl_cville_blkgps.csv")
nfhl_cville_blocks <- read_csv("../dataraw/nfhl_cville_blocks.csv")

# arrange data
nfhl_cville_tracts <- nfhl_cville_tracts %>% 
  dplyr::select(-c(`...1`, spatial_unit)) %>% 
  mutate(GEOID = as.character(GEOID))

nfhl_cville_tracts2 <- nfhl_cville_tracts %>% 
  pivot_wider(names_from = zone, values_from = c(area, perc))

nfhl_cville_blkgps <- nfhl_cville_blkgps %>% 
  dplyr::select(-c(`...1`, spatial_unit)) %>% 
  mutate(GEOID = as.character(GEOID))

nfhl_cville_blkgps2 <- nfhl_cville_blkgps %>% 
  pivot_wider(names_from = zone, values_from = c(area, perc))

nfhl_cville_blocks <- nfhl_cville_blocks %>% 
  dplyr::select(-c(`...1`, spatial_unit)) %>% 
  mutate(GEOID = as.character(GEOID))

nfhl_cville_blocks2 <- nfhl_cville_blocks %>% 
  pivot_wider(names_from = zone, values_from = c(area, perc))

# Join data to geometry
nfhl_cville_tracts_geo <- left_join(cville_tracts, nfhl_cville_tracts2)
nfhl_cville_blkgps_geo <- left_join(cville_blkgps, nfhl_cville_blkgps2)
nfhl_cville_blocks_geo <- left_join(cville_blocks, nfhl_cville_blocks2, by = c("GEOID10" = "GEOID"))
```
...
#### Tracts
```{r}
pal <- colorNumeric("viridis", reverse = TRUE, domain = nfhl_cville_tracts_geo$perc_1per)

leaflet() %>%
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = nfhl_cville_tracts_geo,
              ...
              popup = paste0("FIPS Code: ", nfhl_cville_tracts_geo$GEOID, "<br>",
                             "Percent in 1% floodplain: ", nfhl_cville_tracts_geo$perc_1per)) %>%
  addLegend("bottomright", pal = pal, values = nfhl_cville_tracts_geo$perc_1per,
...
#### Block Groups
```{r}
pal <- colorNumeric("viridis", reverse = TRUE, domain = nfhl_cville_blkgps_geo$perc_1per)

leaflet() %>%
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = nfhl_cville_blkgps_geo,
              fillColor = ~pal(perc_1per),
              weight = 1,
              opacity = 1,
              color = "white",
              fillOpacity = 0.6,
              highlight = highlightOptions(weight = 2, fillOpacity = 0.8, bringToFront = T),
              popup = paste0("FIPS Code: ", nfhl_cville_blkgps_geo$GEOID, "<br>",
                             "Percent in 1% floodplain: ", nfhl_cville_blkgps_geo$perc_1per)) %>%
  addLegend("bottomright", pal = pal, values = nfhl_cville_blkgps_geo$perc_1per,
            title = "Percent in 1% Annual Floodplain", opacity = 0.7)
```
...
#### Tracts
```{r}
pal <- colorNumeric("viridis", reverse = TRUE, domain = nfhl_cville_tracts_geo$perc_2per)
...
leaflet() %>%
  ...
  addPolygons(data = nfhl_cville_tracts_geo,
              ...
              highlight = highlightOptions(weight = 2, fillOpacity = 0.8, bringToFront = T),
              popup = paste0("FIPS Code: ", nfhl_cville_tracts_geo$GEOID, "<br>",
                             "Percent in 2% floodplain: ", nfhl_cville_tracts_geo$perc_2per)) %>%
  addLegend("bottomright", pal = pal, values = nfhl_cville_tracts_geo$perc_2per,
            title = "Percent in 2% Annual Floodplain", opacity = 0.7)
```

#### Block Groups
```{r}
pal <- colorNumeric("viridis", reverse = TRUE, domain = nfhl_cville_blkgps_geo$perc_2per)
...
leaflet() %>%
  ...
  addPolygons(data = nfhl_cville_blkgps_geo,
              ...
              highlight = highlightOptions(weight = 2, fillOpacity = 0.8, bringToFront = T),
              popup = paste0("FIPS Code: ", nfhl_cville_blkgps_geo$GEOID, "<br>",
                             "Percent in 2% floodplain: ", nfhl_cville_blkgps_geo$perc_2per)) %>%
  addLegend("bottomright", pal = pal, values = nfhl_cville_blkgps_geo$perc_2per,
            title = "Percent in 2% Annual Floodplain", opacity = 0.7)
```
