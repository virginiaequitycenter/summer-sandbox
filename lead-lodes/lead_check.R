...
e <- cville %>% summarize(totalelep = sum(ELEP.UNITS))
g <- cville %>% summarize(totalgas = sum(GASP.UNITS))
f <- cville %>% summarize(totalother = sum(FULP.UNITS))
...
cvl_area_burden <- cville_area %>% 
  ...
  summarize(totalinc = sum(HINCP.UNITS),
            totalelep = sum(ELEP.UNITS),
            totalgas = sum(GASP.UNITS),
            totalother = sum(FULP.UNITS)) %>% 
