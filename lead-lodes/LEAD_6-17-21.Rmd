...
```{r}
cville <- cville %>% 
mutate(tract = ifelse(FIP==51540000201, "2.01", ifelse(FIP==51540000202, "2.02", ifelse(FIP==51540000302, "3.02", ifelse(FIP==51540000401, "4.01", ifelse(FIP==51540000402, "4.02", ifelse(FIP==51540000501, "5.01", ifelse(FIP==51540000502, "5.02", ifelse(FIP==51540000600, "6", ifelse(FIP==51540000700, "7", ifelse(FIP==51540000800, "8", ifelse(FIP==51540000900, "9", ifelse(FIP==51540001000, "10", no = "other")))))))))))))
```
...
This is the code I am currently troubleshooting. It works when I run it in R, but I keep getting errors when I knit my Markdown file.
```
...
```
cville %>% group_by(tract) %>% 
burdentab <- xtabs(~ burden) %>% 
prop.table(burdentab) %>% 
  round(digits = 2)
```